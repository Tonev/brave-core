...
TEST(BatAdsCalendarUtilTest, GetLastDayOfMonth) {
  ...
  const std::set<int>& expected_last_day_of_months = {31, 28, 31, 30, 31, 30,
                                                      31, 31, 30, 31, 30, 31};

  // Act
  for (const auto& expected_last_day_of_month : last_day_of_months) {
    const int last_day_of_month = GetLastDayOfMonth(year, month);
...
TEST(BatAdsCalendarUtilTest, GetLastDayOfMonthForLeapYear) {
  ...
  const std::set<int>& expected_last_day_of_months = {31, 29, 31, 30, 31, 30,
                                                      31, 31, 30, 31, 30, 31};
