...
TEST_F(BatAdsTimeUtilTest, GetTimeAsMinutes) {
  ...
  const base::Time& time =
      TimeFromString("November 18 2020 12:34:56", /* is_local */ false);

  // Act
  const int minutes = GetTimeAsMinutes(time);

  // Assert
  const int expected_minutes = (12 * 60) + 34;
...
TEST_F(BatAdsTimeUtilTest, AdjustTimeToBeginningOfPreviousMonth) {
  ...
  const base::Time& expected_adjusted_time =
      TimeFromString("October 1 2020 00:00:00.000", /* is_local */ false));
...
TEST_F(BatAdsTimeUtilTest, AdjustTimeToBeginningOfPreviousMonthOnCusp) {
  // Arrange
  const base::Time& time =
      TimeFromString("January 18 2020 12:34:56.789", /* is_local */ false);
  AdvanceClock(time);
  ...
  // Assert
  const base::Time& expected_adjusted_time =
      TimeFromString("December 1 2019 00:00:00.000", /* is_local */ false));
...
TEST_F(BatAdsTimeUtilTest, AdjustTimeToEndOfPreviousMonth) {
  ...
  // Assert
  const base::Time& expected_adjusted_time =
      TimeFromString("October 31 2020 23:59:59.999", /* is_local */ false));
...
TEST_F(BatAdsTimeUtilTest, AdjustTimeToEndOfPreviousMonthOnTheCusp) {
  ...
  // Assert
  const base::Time& expected_adjusted_time =
      TimeFromString("December 31 2019 23:59:59.999", /* is_local */ false));
...
TEST_F(BatAdsTimeUtilTest, AdjustTimeToBeginningOfMonth) {
  ...
  // Assert
  const base::Time& expected_adjusted_time =
      TimeFromString("November 1 2020 00:00:00.000", /* is_local */ false));
